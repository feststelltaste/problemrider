...
* `list_untranslated_de.py`: Lists `_problems/`/`_solutions/` entries with no German counterpart in `_problems_de/`/`_solutions_de/` (matched via the `en_slug` front-matter key). This is the Phase 5 "Nachzieh-Lauf" from `plans/german-translation-plan.md` — run it periodically, and especially after `/pr:generate_new_problems` or any manual addition, to see what still needs a German translation. Exits non-zero if anything is missing.
