...
ProblemRider is an application that helps software teams dig deeper into the real root causes of their problems and find suitable solutions. Rather than treating surface-level symptoms, it provides a structured catalog of common software development issues, their symptoms, root causes, and applicable solutions. By analyzing how problems interconnect and which practices address them, ProblemRider enables teams to address fundamental issues instead of applying temporary fixes.

See [GEMINI.md](GEMINI.md) for more details.

## Idea

The main idea of this project is to document as many problems and solutions as possible. For this, we use state-of-the-art Large Language Models (SOTA LLMs) such as Google Gemini 2.5 Pro and Anthropic Claude Sonnet 4 to generate and interlink problems and solutions. We also manually check for incorrect connections and hallucinations, and we provide an application that makes this large problem space easy to navigate.
...
- `landscape.html` - Problem/solution landscape page: a clustered UMAP view of both catalogs, with an article panel that opens on click (data generated by `scripts/create_landscape.py`)
...
- **`_solutions/` - Contains markdown files documenting practices, patterns, and tactics that address one or more problems**
- `solutions.html` - HTML template for the solutions listing
- `_includes/` - Jekyll template partials (footer, header, head)
- `_layouts/` - Jekyll page templates (default, problem, solution)
...
- `solution-pattern-template.md` - Template for creating a new solution
- `quality-tactics-reference.md` - Reference of Quality Tactics that can be used for solutions
...
- `scripts/` - Python utilities for maintenance, analysis, and visualization:
  ...
  - `create_landscape.py` - Generates the UMAP-based clustered layout data for the problem/solution landscape page
  - `find_unlinked_content.py` - Identifies causes and symptoms without markdown links
  - `validate_causal_links.py` - Checks the Symptoms/Causes graph for contradictions, asymmetric claims, and cycles
  ...
  - `sync_problem_solution_links.py` - Keeps problem-to-solution links consistent in both directions
  - `sync_quality_tactics.py` - Syncs Quality Tactics titles and descriptions into solution front matter
...
Contributions to the underlying catalog of ProblemRider as well as the app itself are very welcome.

To add a new problem pattern, please follow these steps:
...
To add a solution, use the [`solution-pattern-template.md`](./solution-pattern-template.md) file and follow these steps:

1. Browse the existing solutions in the `_solutions` directory.
2. Create a new, lowercase, hyphen-separated Markdown file in `_solutions`.
3. Add the problems addressed by the solution to its `problems:` list in the front matter.
4. If the solution corresponds to a Quality Tactics tactic, add its `quality_tactics_url`. Otherwise, include a `## Description` section in the body.
5. Add practical guidance in the `How to Apply`, `Tradeoffs`, and `How It Could Be` sections.
6. Run `python scripts/sync_problem_solution_links.py` to update links in both directions.
