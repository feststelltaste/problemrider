...
Ensemble programming, also known as mob programming, brings an entire small group of developers together at one shared workstation and screen to work on a single problem simultaneously, with one person acting as "driver" typing at the keyboard while the rest of the group navigates, discusses, and directs, rotating the driver role every ten to fifteen minutes. Rather than splitting a problem across individuals who each hold a partial view of it, the technique deliberately pools everyone's knowledge into the same moment, so that whatever the group produces reflects the combined understanding of everyone present rather than the limited perspective of whoever happened to pick up the task. This is especially suited to the parts of a legacy system that no single developer fully understands, because responsibility for the code passed through several different teams over the years without documentation, and understanding it requires assembling fragments of knowledge that are individually scattered across different people. Working through such code as an ensemble turns what would otherwise be a slow, solitary reverse-engineering effort — or a bug that stays unresolved because it falls between areas of expertise — into a session where people with complementary partial knowledge combine it in real time, often reaching an understanding that no one of them could have reached alone. It is also an effective, high-bandwidth way to onboard a new team member directly into the legacy codebase's hardest areas, immersing them in context that would otherwise take months to accumulate independently. The obvious cost is that it consumes several developers' time at once for a single piece of work, which can look inefficient by conventional individual-output measures, and it is best reserved for genuinely difficult, high-stakes problems rather than applied to routine work that a single developer would handle just as well alone.
